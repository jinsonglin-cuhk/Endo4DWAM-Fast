...
### 给生成侧：flow 应在训练几何下计算（实测）

用**前后向一致性**（reference-free，不预设任何一方为真值）比较三种计算分辨率，越小越好：

| 帧对 | 类型 | 训练变换 256×320 | 压缩 256×320 | 原图 360×480 | 现方案 224×224 |
|---|---|---|---|---|---|
| 1245→1246 | 高运动 | **0.0330** | 0.0370 | 0.0371 | 0.0511 |
| 1340→1341 | 高运动 | 0.1512 | **0.0895** | 0.1202 | 0.2479 |
| 900→901 | 近静止 | **0.1300** | 0.1466 | 0.2793 | 0.2629 |
| 600→601 | 近静止 | 0.4824 | **0.1492** | 0.5612 | 0.4917 |
| 200→201 | 近静止 | **0.3705** | 0.4193 | 0.6586 | 0.9591 |

两点结论：

1. **256×320 全面优于原图 360×480 和现方案 224×224**。最优点不是原始分辨率——RAFT 的
   `C_T_SKHT_V2` 在更接近其训练尺度处表现更好；224×224 还额外把 0.75 的宽高比压成 1.0，全场最差。
2. **在训练几何下计算，flow 就不需要任何向量变换**。若在原图 360×480 计算，因为等比缩放+中心裁剪
   只裁宽度，必须 `u × 1.066667` 而 `v` 不变；漏掉不报错，只让水平运动目标系统性偏小 6.7%。

因此建议生成侧：帧先过训练管线的 `ResizeSmallestSideAspectPreserving + CenterCrop` 到 256×320，
再跑 RAFT，按 `t→t+1`，存归一化单位 `(u/W, v/H)`。存储分辨率可降到 128×160（归一化单位与分辨率无关）：
全量 979,564 帧在 128×160 fp16 约 **75 GB**，256×320 则约 299 GB（`/mnt/data2` 现剩 1.5 TB）。

另建议 flow 数组按**每帧一项**存、末帧置零并 mask=0，而不是存 T−1 项——后者会让 flow 与视频帧号
差一位，是典型的静默错位来源。
