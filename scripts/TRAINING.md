...
本文档说明如何在 EndoWAM 内镜数据集上训练 Endo4DWAM 系列模型。
...
|---|---|---|---|
| **Endo4DWAM** (base) | `train_endowam_lora_uncond.sh` | `endowam_uncond_1cam_1e-4` | action tokens 只 attend **第一帧** video latent；推理时可缓存视频 K/V 加速 |
| **Endo4DWAMJoint** | `train_endowam_lora_joint.sh` | `endowam_joint_1cam_1e-4` | action tokens attend **完整** video 序列；更强的 baseline |
| **Endo4DWAMIDM** | `train_zero1.sh` + IDM task | `endowam_idm_*` | 两阶段推理：先去噪视频再 condition 动作 |

**推荐起点**：`Endo4DWAM` (base) — 对应 `train_endowam_lora_uncond.sh`。
...
│   ├── model/
│   │   ├── endo4dwam.yaml        # Endo4DWAM base 模型配置（含 LoRA 默认值）
│   │   ├── endo4dwam_joint.yaml  # Endo4DWAMJoint 模型配置
│   │   └── endo4dwam_idm.yaml    # Endo4DWAMIDM 模型配置
│   └── task/
│       ├── endowam_uncond_1cam_1e-4.yaml   # EndoWAM base 任务配置
│       └── endowam_joint_1cam_1e-4.yaml    # EndoWAM joint 任务配置
├── scripts/
│   ├── TRAINING.md                       # 本文档
│   ├── train_endowam_lora_uncond.sh      # Endo4DWAM base + LoRA 训练脚本
│   ├── train_endowam_lora_joint.sh       # Endo4DWAMJoint + LoRA 训练脚本
...
Endo4DWAM 的 LeRobot 数据加载器需要每个子集下存在 `meta/episodes_stats.jsonl`。
...
### 4.1 Endo4DWAM base（推荐起点）

训练**基础 Endo4DWAM** 模型（action 只 attend 第一帧）：
...
训练 **Endo4DWAMJoint**（action attend 完整视频序列）：
...
Endo4DWAM 的 LoRA 只注入到 **视频 expert（WanVideoDiT 5B）**，Action expert 保持全参数微调：
...
| 配置 | 显存需求（单 GPU） |
|---|---|
| Endo4DWAM base，batch=4，ZeRO-1，bf16，无梯度检查点 | ~40–48 GB |
