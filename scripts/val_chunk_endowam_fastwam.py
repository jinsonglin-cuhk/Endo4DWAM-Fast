"""
FastWAM 离线开环 chunk 评测（EndoWAM 内窥镜数据集）。

对齐 EndoWAM/tests/val_chunk_endowam_casual.py 的评测方式：加载一个训练好的
FastWAM checkpoint，对某个 LeRobot episode 做开环（GT state / 真实首帧锚定）
...
复用点（不重复实现）：
  - `fastwam.trainer.Wan22Trainer._to_batched_eval_sample`（staticmethod）：把
  ...
  - `model.infer_action` / `model.infer` / `model.training_loss`：与
    `experiments/robotwin/fastwam_policy/deploy_policy.py`（闭环部署）和
...
与 EndoWAM 参考脚本的差异：
  - FastWAM 是 video+action 联合 flow-matching 模型，不是离散分类模型，所以
...
用法：
    ...
    cd /mnt/data2/ljs/FastWAM
    python scripts/val_chunk_endowam_fastwam.py \
...
from fastwam.utils.video_metrics import (  # noqa: E402
...
def plot_chunk_results(gt: np.ndarray, pred: np.ndarray, exp_tag: str, save_path: Path,
    ...
    fig.suptitle(f"{exp_tag}\nFastWAM chunk validation (receding-horizon, GT-anchored)", fontsize=14)
...
def main() -> None:
    ...
    print(f"[model] building FastWAM (dtype={model_dtype}, device={device}) ...")
