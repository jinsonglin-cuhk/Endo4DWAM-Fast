...
from .endo4dwam import Endo4DWAM

logger = get_logger(__name__)


class Endo4DWAMJoint(Endo4DWAM):
    """Endo4DWAM variant where action attends to all video latent tokens."""
    ...
    def from_wan22_pretrained(cls, **kwargs):
        ...
        if not isinstance(video_dit_config, dict):
            raise ValueError(
                "`video_dit_config` must be provided as dict for Endo4DWAMJoint."
            )
        if bool(video_dit_config.get("action_conditioned", False)):
            raise ValueError(
                "Endo4DWAMJoint requires `video_dit_config['action_conditioned']=false`."
            )
    ...
    def infer_joint(
        ...
        if test_action_with_infer_action:
            logger.warning(
                "`Endo4DWAMJoint.infer_joint` ignores `test_action_with_infer_action=True` "
