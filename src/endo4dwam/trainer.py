...
class Wan22Trainer:
    ...
    def _apply_dit_only_train_mode(model):
        ...
        if getattr(model, "_lora_enabled", False) and not getattr(model, "_lora_train_base", False):
            # Video expert: only LoRA params (lora_A / lora_B) are trainable; base weights frozen.
            # Action expert: all params are fully trainable (analogous to EndoWAM's action decoder).
            for n, p in model.dit.named_parameters():
                if "mixtures.video." in n and "lora_" not in n:
    ...
    def _rotate_checkpoints(self):
        """Keep only the last save_total_limit weight .pt files and state dirs.

        Mirrors EndoWAM VLATrainer._rotate_checkpoints but handles both the
        weights/ and state/ sub-directories that Endo4DWAM produces.
        """
        ...
        try:
            weight_files = [
                f for f in os.listdir(self.weights_dir)
                if re.match(r"^step_(\d+)\.pt$", f)
                and os.path.isfile(os.path.join(self.weights_dir, f))
            ]
        except FileNotFoundError:
            weight_files = []

        if len(weight_files) > self.save_total_limit:
            weight_files.sort(key=lambda f: int(re.search(r"step_(\d+)\.pt", f).group(1)))
            for fname in weight_files[:-self.save_total_limit]:
                fpath = os.path.join(self.weights_dir, fname)
                try:
                    os.remove(fpath)
                    logger.info("Removed old weight checkpoint: %s", fpath)
                except OSError as e:
                    logger.warning("Failed to remove weight checkpoint %s: %s", fpath, e)
        ...
        try:
            state_dirs = [
                ...
                and os.path.isdir(os.path.join(self.state_dir, d))
            ]
        except FileNotFoundError:
            state_dirs = []

        if len(state_dirs) > self.save_total_limit:
            state_dirs.sort(key=lambda d: int(re.search(r"step_(\d+)", d).group(1)))
            for dname in state_dirs[:-self.save_total_limit]:
                dpath = os.path.join(self.state_dir, dname)
                try:
                    shutil.rmtree(dpath)
                    logger.info("Removed old state checkpoint: %s", dpath)
                except OSError as e:
                    logger.warning("Failed to remove state checkpoint %s: %s", dpath, e)
